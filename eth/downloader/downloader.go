...
func (d *Downloader) processHeaders(origin uint64, td *big.Int) error {
	...
	defer func() {
		if rollback > 0 {
			...
			if rollbackErr == errCanceled {
				log.Warn("Skip rolling back chain segment",
					"header", fmt.Sprintf("%d->%d", lastHeader, rollback-1),
					"fast", fmt.Sprintf("%d", lastFastBlock),
					"block", fmt.Sprintf("%d", lastBlock))
				return
			}
